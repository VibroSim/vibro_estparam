c     This software in public domain because it was part of SLATEC
      subroutine dqagse(f,fp1,fp2,fp3,fp4,a,b,epsabs,epsrel,limit,
     *    result,abserr,neval,
...
c
c            fp1    - double parameter #1 for f
c            fp2    - double parameter #2 for f
c            fp3    - double parameter #3 for f
c            fp4    - double parameter #4 for f
...
c
     ...
     *  res3la,rlist,rlist2,small,uflow,fp1,fp2,fp3,fp4
...
c
      ...
      oflow = d1mach(2)
      ierro = 0
      call dqk21(f,fp1,fp2,fp3,fp4,a,b,result,abserr,defabs,resabs)
c
c           test on accuracy.
c
      dres = dabs(result)
...
c
        ...
        b2 = blist(maxerr)
        erlast = errmax
        call dqk21(f,fp1,fp2,fp3,fp4,a1,b1,area1,error1,resabs,defab1)
        call dqk21(f,fp1,fp2,fp3,fp4,a2,b2,area2,error2,resabs,defab2)
c
...
c
  ...
  999 return
      ...
      subroutine dqagie(f,fp1,fp2,fp3,fp4,bound,inf,epsabs,epsrel,
     *   limit,result,abserr,
...
c
      ...
      call dqk15i(f,fp1,fp2,fp3,fp4,boun,inf,0.0d+00,0.1d+01,result,
     *  abserr,
     *  defabs,resabs)
c
c           test on accuracy
...
c
        ...
        b2 = blist(maxerr)
        erlast = errmax
        call dqk15i(f,fp1,fp2,fp3,fp4,boun,inf,a1,b1,area1,error1,
     *    resabs,defab1)
        call dqk15i(f,fp1,fp2,fp3,fp4,boun,inf,a2,b2,area2,error2,
     *     resabs,defab2)
c
c           improve previous approximations to integral
c           and error and test for accuracy.
...
c
  ...
  999 return
      ...
      subroutine dqk15i(f,fp1,fp2,fp3,fp4,
     *  boun,inf,a,b,result,abserr,resabs,resasc)
...
c
      ...
      if(inf.eq.2) fval1 = fval1+f(-tabsc1,fp1,fp2,fp3,fp4)
...
c
      ...
      do 10 j=1,7
        ...
        if(inf.eq.2) fval1 = fval1+f(-tabsc1,fp1,fp2,fp3,fp4)
        if(inf.eq.2) fval2 = fval2+f(-tabsc2,fp1,fp2,fp3,fp4)
...
c
  ...
  100 abserr = dmax1(abserr,0.5d+01*epmach*dabs(result))
      ...
      subroutine dqk21(f,fp1,fp2,fp3,fp4,a,b,result,abserr,resabs,
     *   resasc)
...
c
     ...
     *  resg,resk,reskh,result,uflow,wg,wgk,xgk,fp1,fp2,fp3,fp4
      integer j,jtw,jtwm1
      external f
c
...
c
      ...
      do 10 j=1,5
        jtw = 2*j
        absc = hlgth*xgk(jtw)
        fval1 = f(centr-absc,fp1,fp2,fp3,fp4)
        fval2 = f(centr+absc,fp1,fp2,fp3,fp4)
        fv1(jtw) = fval1
        fv2(jtw) = fval2
        fsum = fval1+fval2
        resg = resg+wg(j)*fsum
